...
<div class="cv-store-wrap">
    ...
    <?php if ($justAdded): ?>
        <div class="cv-alert cv-alert-success mb-4">
            ...
            <a href="cart.php" style="color:var(--cv-accent);font-weight:600;">View Cart &rarr;</a>
        </div>
    <?php endif; ?>
    ...
    <?php if (empty($grouped)): ?>
        ...
        <div class="text-center py-5">
            <i class="bi bi-inboxes" style="font-size:3rem;color:var(--cv-muted);"></i>
            <h4 class="mt-4 fw-bold" style="color:var(--cv-text);">No chairs available right now</h4>
            <p style="color:var(--cv-muted);">Please check back soon.</p>
        </div>
        ...
        <aside class="cv-store-sidebar">
            ...
            <nav class="cv-sidebar-nav">
                <?php foreach ($grouped as $categoryName => $items):
                    $catIcon = isset($catIcons[$categoryName]) ? $catIcons[$categoryName] : 'bi-person-workspace';
                ...
                ?>
                    <button type="button"
                        ...
                        <span class="cv-sidebar-icon"><i class="bi <?= $catIcon ?>"></i></span>
        ...
        <div class="cv-store-main">
            ...
            <div class="cv-content-toolbar">
                <div class="cv-content-heading">
                    <?php $catIcon = isset($catIcons[$firstCategory]) ? $catIcons[$firstCategory] : 'bi-person-workspace'; ?>
                    <div class="cv-category-icon">
                        <i class="bi <?= $catIcon ?>" id="cv-active-cat-icon"></i>
                    </div>
                    <div>
                ...
                <div class="cv-cat-section<?= $isFirst ? ' active' : '' ?>"
                    ...
                    <div class="cv-product-grid">
                        <?php foreach ($items as $p):
                            ...
                            $prodIcon = isset($catIcons[$categoryName]) ? $catIcons[$categoryName] : 'bi-person-workspace';
                        ?>
                                ...
                                <a href="product.php?id=<?= (int)$p['id'] ?>" class="cv-product-link">
                                    <div class="cv-product-thumb">
                                        <?php if (!empty($p['image'])): ?>
                                            ...
                                            <i class="bi <?= $prodIcon ?>" style="display:none;"></i>
                                        <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function selectCategory(item) {
        ...
        sections.forEach(function (section) {
            if (section.getAttribute('data-category') === target) {
                ...
                if (activeName) activeName.textContent = text;
                if (activeCount) activeCount.textContent = countVisible(section) + ' items';
            } else {
    ...
    var wantedCat = params.get('cat');
    if (wantedCat && sidebarItems.length) {
        var encodedWanted = encodeURIComponent(wantedCat).replace(/%20/g, '+');
        var match = null;
        sidebarItems.forEach(function (item) {
            if (!match && item.getAttribute('data-target') === encodedWanted) {
                match = item;
            }
        });
