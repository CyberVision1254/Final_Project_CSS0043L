...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        $desc_esc = mysqli_real_escape_string($conn, "$full_name registered a new buyer account.");
        ...
        $emailSent = sendVerificationEmail($email, $full_name, $token);

        if ($emailSent) {
            $_SESSION['success'] = "Registration successful! Please check your email (" . htmlspecialchars($email) . ") to confirm your account before logging in.";
        } else {
            $_SESSION['success'] = "Registration successful, but we couldn't send the confirmation email. Please contact support to verify your account.";
        }
