...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $errors = array();
    if (empty($name_raw))                                $errors[] = "Chair name is required.";
    if (empty($category_raw))                            $errors[] = "Category is required.";
    ...
    if (!empty($errors)) {
        ...
        if ($id == 0) {
            $insertOk = mysqli_query($conn, "INSERT INTO products (name,category,description,price,stock_qty,is_active)
                                             VALUES ('$name','$category','$description',$price_val,$stock_val,$is_active)");
            if ($insertOk) {
                $uid   = (int)$_SESSION['user_id'];
                $aname = mysqli_real_escape_string($conn, $_SESSION['fullname']);
                $desc  = mysqli_real_escape_string($conn, $_SESSION['fullname'] . " added chair: $name_raw (stock: $stock_val, price: ₱$price_val).");
                ...
                $message = "Chair \"$name_raw\" was added successfully.";
                $type    = "success";
            } else {
                $message = "Database error while adding the chair: " . mysqli_error($conn);
                $type    = "danger";
            }
        } else {
            $old_row  = mysqli_fetch_assoc(mysqli_query($conn, "SELECT price,stock_qty FROM products WHERE id=$id"));
            $updateOk = mysqli_query($conn, "UPDATE products SET name='$name',category='$category',description='$description',price=$price_val,stock_qty=$stock_val,is_active=$is_active WHERE id=$id");
            ...
            if ($updateOk) {
                ...
                $uid   = (int)$_SESSION['user_id'];
                $aname = mysqli_real_escape_string($conn, $_SESSION['fullname']);
                $desc  = mysqli_real_escape_string($conn, $_SESSION['fullname'] . " updated chair: $name_raw.$change");
                ...
                $message = "Chair \"$name_raw\" was updated.";
                $type    = "success";
            } else {
                $message = "Database error while updating the chair: " . mysqli_error($conn);
                $type    = "danger";
            }
        }
    }
